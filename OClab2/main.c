...
int main()
{
    ...
    //- Что такое "переменная окружения"? где она хранится и для чего служит?
    ...
    //- Где и как можно узнать как задаётся любой произвольный часовой пояс?
    ...
    (void) time( &now ); //time записывает в now значение времени  в секундах  от 1 января 1970, 00:00:00 UTC, при неуспешном завершении записывает -1
     ...
     //- Функция меняет переменную окружения TZ во всей ОС, или действие ограничено? если да, то чем и причём тут shell?
